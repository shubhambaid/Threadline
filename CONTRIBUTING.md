...
Run the CLI from source with `npm run build && node dist/cli.js <command>`, or `npm link` to put `threadline` on your PATH.

CI runs all four scripts on Linux and macOS, on Node 22 and 24.

## Project rules

1. **The spec is normative.** A change to the record format updates [docs/spec.md](docs/spec.md), the JSON Schemas in [schemas/](schemas/), and the tagged examples in the spec together. `test/spec/spec-examples.test.ts` validates every example marked `<!-- threadline:schema=… -->`, so docs and schemas cannot drift apart.
...
3. **Deterministic output.** Commands take the time from `THREADLINE_NOW` and identity from `THREADLINE_AGENT`, sort everything by stable keys, and produce byte-identical output for identical input. `resume` never uses embeddings or model calls.
...
- `test/fixtures/<name>/`: repositories described as commits plus `.threadline/` files, with `{{commit:N}}` placeholders.
...
- Describe what changed and why. `threadline render pr-summary` can help if you used Threadline for the work.
