# Threadline with Claude Code

Claude Code can use Threadline through its instruction file (`CLAUDE.md`), through the CLI in its shell, and through MCP.
...
`@path` imports are resolved relative to the importing file. `threadline render claude-md` recognizes the import and does not write a second copy of the block.

**Or give Claude Code its own block:**

```sh
threadline render claude-md            # preview
threadline render claude-md --write    # add or update it in CLAUDE.md
threadline render claude-md --check    # in CI: exit 1 if missing or out of date
```

Only the text between `<!-- threadline:begin -->` and `<!-- threadline:end -->` is managed. `render` refuses to write through a `CLAUDE.md` symlink to `AGENTS.md`; run `render agents-md --write` instead.
...
The CLI is enough for an agent that can run shell commands. MCP additionally gives Claude Code typed tools (`resume`, `status`, `validate`, `task_start`, `task_claim`, `checkpoint_create`, `receipt_record`, `decision_add`, `knowledge_add`) and resources (`threadline://status`, `threadline://records/{id}`).

Share the server with everyone working on the repository (writes `.mcp.json`):

```sh
claude mcp add --scope project --transport stdio --env THREADLINE_AGENT=claude-code threadline -- threadline mcp
```
...
```json
{
  "mcpServers": {
    "threadline": {
      "command": "threadline",
      "args": ["mcp"],
      "env": { "THREADLINE_AGENT": "claude-code" }
    }
  }
}
```
...
Claude Code passes `CLAUDE_PROJECT_DIR` to the servers it starts, and `threadline mcp` uses it to find the repository. An explicit `-C /path/to/repo` (`args = ["-C", "/path/to/repo", "mcp"]`) takes precedence.
